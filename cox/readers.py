...
class CollectionReader:
    ...
    def df(self, key, append_exp_id=True, keep_serialized=[], union_schemas=False, exp_filter=None, skip_errors=False):
        tables = []
        schema = None
        ...
        for exp_id, store in self.stores.items(): #exp_id in tqdm.tqdm(os.listdir(self.directory)):
            # store_path = os.path.join(self.directory, exp_id, STORE_BASENAME)
            # if not os.path.exists(store_path):
            #     continue
